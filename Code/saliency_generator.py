...
class SaliencyGenerator:
    ...
    def __init__(self, input_path, output_path, temporal_ROI):
        bkg = Image.open(os.path.join(input_path, "bkg.jpg"))
        self.learning(bkg)
        self.generate(input_path, output_path, temporal_ROI)
    ...
    def generate(self, input_path, output_path, temporal_ROI):
        ...
        for i in range(temporal_ROI[0], temporal_ROI[1]+1):
            ...
            current = Image.open(os.path.join(input_path, "in{0:06d}.jpg".format(i)))
...
#             diff_winners *= 30  # Use this parameter ?
            ...
            # Saving
            som_difference_modulated.save(os.path.join(output_path, "saliency", "sal{0:06d}.png".format(i)))
            result.save(os.path.join(output_path, "results", "bin{0:06d}.png".format(i)))


if __name__ == '__main__':
    cdnet_path = "/users/yabernar/GrosDisque/CDNET14/dataset/baseline/pedestrians/input/"
    output_path = "/users/yabernar/GrosDisque/CDNET14/testing"
    roi = (300, 330)
    SaliencyGenerator(cdnet_path, output_path, roi)
