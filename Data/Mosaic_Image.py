...
class MosaicImage:
    ...
    #
    # def load_single_channel(self, path, channel):
    #     self.data = []
    #     im = Image.open(path)
    ...
    #         px = np.hsplit(px, 3)[channel]
    #     px = px.reshape(size)
    #     self.nb_pictures = np.array(np.divide(size, pictures_dim), dtype=int)
    #     px = px[0:self.nb_pictures[0] * pictures_dim[0],
    #          0:self.nb_pictures[1] * pictures_dim[1]]  # Cropping the image to make it fit
    #     px = np.vsplit(px, self.nb_pictures[0])
    #     for i in px:
    #         j = np.hsplit(i, self.nb_pictures[1])
    #         for picture in j:
